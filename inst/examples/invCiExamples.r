...
plot(dat,ylim=c(0.05,0.55),refsize=4,las=1,xlim=c(-1,6),main="Inverse-Estimation CIs") # uses plot.doseResponse()
...
# To get "global" values calculated for you at specific targets, choose 'delta=FALSE' when calling quickInverse()
lines(quick1$lower90conf,lty=2,col=3) 
lines(quick1$upper90conf,lty=2,col=3) 
# Note how for y=0.3, both bounds are infinite (i.e., no intersection with the horizontal line) unless one dares to extrapolate outside range of observations.
...
legend('topleft',pch=c(NA,'X',NA,NA),lty=c(1,NA,2,2),col=c(4,1,1,3),legend=c('True Curve','Observations','Local Interval (default)','Forward/Global Interval'),bty='n')
