...
The lower 83% confidence limit of ropivacaine is almost touching the upper 83% limit levobupivacaine, but there’s no overlap. One could conclude that there is moderate evidence for a difference in potency at the $\alpha=0.05$ level.
Thus, our conclusion is similar to Pace and Stylianou’s reanalysis, but not as strong. Our point estimates are also very similar to theirs, and so is the levo/ropi potency ratio (1.37, quite a bit higher than Benhamou et al.’s 1.19). This should not be too surprising, because CIR is an extension of IR which Pace and Stylianou used, whereas Benhamou et al. used dose averaging. 
...
Note the argument specifying the `oldPAVA()` estimation function (PAVA is the name of the leading algorithm to produce IR estimates). 
...
Each of these returns a list of three `doseResponse` objects. Here's one:
```{r forward2}
ropiCurveCIR
```

If you want a quick, "short-form" forward estimate -- the analogue of `quickInverse()` used above for inverse estimation -- use `quickIsotone`. You will get an output similar to the `$output` object, together with confidence intervals for each dose. That function's arguments are similar to `quickInverse()`.

We end with the dose-response plots.

```{r drfig,fig.width=9,fig.height=4.5,out.height=400,out.width=800}
par(mfrow=c(1,2), las=1, mar=c(4,4,4,1)) # image format parameters
