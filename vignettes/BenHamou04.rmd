...
To obtain a quick, "short-form" forward estimate -- the forward analogue of `quickInverse()` used above -- use `quickIsotone()`. You will get an output similar to the `$output` object above, together with confidence intervals for each dose. That function's arguments are similar to `quickInverse()`.
